...
//modify by shf675 opts
function _4e(_4f,_50,opts){
function _51(){
var _52=[];
var _53=[];
...
/*Start:add by shf675*/
var frozenAlign = opts.frozenAlign ? opts.frozenAlign:"left";
if(frozenAlign == "left"){
_57.css("left", "0");
_58.css("right", "0");
} else {
...
//Start:1、默认可排序；2、不可排序时隐藏排序图标 add by shf675
if(col.sortable==undefined){
col.sortable=true;
}
if(!col.sortable){
$("span.datagrid-sort-icon",td).removeClass("datagrid-sort-icon");
}
//End:add by shf675
...
//Start:add by shf675 未设置宽度时给个默认宽度，防止自适应低版本浏览器卡顿的情况
if(!col.width){
col.width = 100;
}
//End:add by shf675
...
//Start:获取contextpath add by shf675
...
//add by shf675  add opts
var _1b7=_4e(this,opts.rownumbers,opts);
if(!opts.columns){
...
//Start:导出 add by shf675 
bindExport:function(datagrid, opts){//绑定导出
	if(!opts) return false;
	//绑定导出全部
	var exportId = datagrid.attr("exportId");
	if (exportId) {
		var exportBtn = $("#" + exportId);
		var exportName = exportBtn.attr("exportName");
		var isAsyn = exportBtn.attr("isAsyn");
		var asynExportWay = exportBtn.attr("asynExportWay");
		var dateConfig = exportBtn.attr("dateConfig");
		...
		if(isAsyn){
			opts.isAsyn = isAsyn;
			opts.asynExportWay = asynExportWay ? asynExportWay:1;//如未指定，默认为即时导出
		}
		
		if(dateConfig){
			opts.dateConfig = dateConfig;
		}
		
		//绑定目标导出对象
		exportBtn.unbind('click').bind('click', function() {
			datagrid.datagrid("exportData", opts);
		});
	}
	
	//绑定导出当页
	var exportCurrentId = datagrid.attr("exportCurrentId");
	if (exportCurrentId) {
		var exportBtn = $("#" + exportCurrentId);
		var exportName = exportBtn.attr("exportName");
		...
		//绑定目标导出对象
		exportBtn.unbind('click').bind('click', function() {
			datagrid.datagrid("exportCurrentData", opts);
		});
	}
},
//导出全部
exportData:function(jq, opts){
	return jq.each(function(){
		var dg = $.data(this, "datagrid");
		var url = opts.url;
		var queryParams = opts.queryParams;
		var isAsyn = opts.isAsyn;
		var asynExportWay = opts.asynExportWay;
		var dateConfig = opts.dateConfig;
		if(asynExportWay == '0'){
			$.messager.alert("错误", "系统暂时禁止导出操作！", "error");
			return;
		}
		
		//1)组装表头配置信息
		var allColumns = $.extend(true, [], opts.columns);
		if (allColumns[0] && opts.frozenColumns && opts.frozenColumns[0]) {
			for (var i = opts.frozenColumns[0].length - 1; i >= 0; i--) {
				var frozenColumn = $.extend(true, {}, opts.frozenColumns[0][i]);
				allColumns[0].unshift(frozenColumn); //插到第一位
			}
		}
		
		//2)删除导出用不到的属性
		try{
			var exportFields = ['field','title','colspan','rowspan','freeze'];
			for(var i=0; i<allColumns.length; i++){
				for(var j=0; j<allColumns[i].length; j++){
					var cc = allColumns[i][j];
					if(cc && cc.exportable == false) {
						delete allColumns[i][j];
						continue;
					}
					
					//删除导出用不到的属性
					$.each(cc, function(name, value){
						if($.inArray(name, exportFields) == -1){
							delete cc[name];
						}
					});
				}
		...
		//3)生成导出表头配置
		var headerJSON = $.object2String(allColumns);
...
//导出当页
exportCurrentData : function(jq, opts){
	return jq.each(function(){
		var tTable = $("#"+opts.id);
		var dg = $.data(this, "datagrid");
		$.messager.confirm("确认", "确定导出吗？", function(ok){
			if(ok){
				$('#expTemp').remove();
			    $("<table id='expTemp'><tr><td id='expLeft'></td><td id='expRight'></td></tr></table>").appendTo($(document.body));
			    $('#expTemp').css({
			        'position': 'absolute',
			        'top': '-9000px'
			    });
			    var view1 = tTable.parents(".datagrid-view").find(".datagrid-view1");
			    var view2 = tTable.parents(".datagrid-view").find(".datagrid-view2");
			    if(view1.find(".datagrid-body").find("tr").size() > 0){
				    $('#expLeft').html(view1.outerHTML());
					//移除被合并隐藏的单元格
				    $('#expLeft').find('td:hidden').remove();
			    }else {
			    	$('#expLeft').remove();
			    }
			    $('#expRight').html(view2.outerHTML().replace(new RegExp('<span class="tree-indent"></span>',"gmi"), '&nbsp;&nbsp;'));
				
				//移除超级链接和span包裹
				$('#expTemp').find('a,span').each(function(){
					$(this).replaceWith($(this).text());
				});
				//移除图片,span
				$('#expTemp').find('img').remove();
				
				//移除class属性等
			    $('#expTemp').find('tr,td,div').removeAttr('class').addClass('temp').removeClass('temp');
			    var data = $('#expTemp').outerHTML();
			
				//后台导出当页方法
				var param = '';
				if(dg.data && dg.data.total){
					param += "&exportTotal="+dg.data.total;//导出总数
				}
				var url = _getContextPath() + '/fileserver/exportCurrentPage?' + param;
				
			    $('#expTemp').empty();
			    $("#expTemp").append('<iframe name="IF_4down" width="0" height="0"></iframe>').append('<form id="exportform" action="'+url+'" method="post" target="IF_4down"><input type="hidden" id="pData" name="pData"/><input type="hidden" id="exportName" name="exportName"/></form>');
			    $("#pData").val(data);
				if(opts.exportName){
					$("#exportName").val("当页导出："+opts.exportName);
				} else {
					$("#exportName").val("当页导出");
				}
...
//Start:合并单元格  add by dq
mergeRowsByfields:function(jq, options){
	return jq.each(function(){
		if(options == null){
			return;
		}
		
		var ColArray;//需合并单元格的列名
		if(typeof options == 'string'){
			ColArray = options.split(",");
		}else if(typeof options == 'object' && typeof options.length == 'number'){
			ColArray = options;
		}else{
			if(typeof options.fields == 'string'){
				ColArray = options.fields.split(",");
			}else if(typeof options.fields == 'object' && typeof options.fields.length == 'number'){
				ColArray = options.fields;
			}
		}
		
		var tTable = $(this);
		var maxRowSize = tTable.datagrid("getRows").length; 
		//起始序列
		var index = options.rowIndex == null ? 0 : options.rowIndex;
		if(index >= maxRowSize-1){
			return;
		}
		
		//待检测是否需合并的行数		
		var offset;
		if(options.rowSpan == null || options.rowSpan + index >= maxRowSize){
			offset = maxRowSize - index;
		}else{
			offset = options.rowSpan;
		}
...
mergeRowsByTreefields:function(jq, fields){
	return jq.each(function(){
		if(fields == null){
			return;
		}
		...
		if(typeof fields == 'string'){
			ColArray = fields.split(",");
		}else if(typeof fields == 'object' && typeof fields.length == 'number'){
			ColArray = fields;
		}
		var tTable = $(this);
		var maxRowSize = tTable.datagrid("getRows").length;
		var rowspanArray = [{index:0, rowspan:maxRowSize}];
...
//Start: add tip add by shf675
var _title='';
if(col.tip==true && _227){
try{
_title = _227.replace(/"/g, '&quot;').replace(/'/g, "&apos;");
}catch(e){}
}
cc.push(col.checkbox?"class=\"datagrid-cell-check\"":"title=\""+(col.tip==true?_title:"")+"\"class=\"datagrid-cell "+col.cellClass+"\"");
//End:add by shf675
cc.push(">");
if(col.checkbox){
...
//Start:绑定导出 add by shf675
$.fn.datagrid.methods.bindExport($(this), opts);
//End:add by shf675
